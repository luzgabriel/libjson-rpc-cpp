...
HttpServer::HttpServer(int port, const std::string &sslcert,
                       const std::string &sslkey, const std::string &sslkeypwd,
                       const std::string &ssltrust, int threads)
    : AbstractServerConnector(), port(port), threads(threads), running(false),
      path_sslcert(sslcert), path_sslkey(sslkey), path_sslkeypwd(sslkeypwd), path_ssltrust(ssltrust),
      daemon(NULL) {}
...
// are deprecated.
    ...
    if (this->path_sslcert != "" && this->path_sslkey != "") {
      try {
        ...
        if(this->path_ssltrust != "") {
          SpecificationParser::GetFileContent(this->path_ssltrust, this->ssltrust);

          if(this->path_sslkeypwd != "") {
            SpecificationParser::GetFileContent(this->path_sslkeypwd, this->sslkeypwd);
            this->daemon = MHD_start_daemon(
                MHD_USE_SSL | mhd_flags, this->port, NULL, NULL,
                HttpServer::callback_tls, this,
                MHD_OPTION_HTTPS_MEM_KEY, this->sslkey.c_str(), 
                MHD_OPTION_HTTPS_KEY_PASSWORD, this->sslkeypwd.c_str(),
                MHD_OPTION_HTTPS_MEM_CERT, this->sslcert.c_str(),
                MHD_OPTION_HTTPS_MEM_TRUST, this->ssltrust.c_str(),
...
int HttpServer::callback_tls(void *cls, MHD_Connection *connection, const char *url,
  ...
  if (*con_cls == NULL) {
    ...
    *con_cls = client_connection;
    const MHD_ConnectionInfo *ci = MHD_get_connection_info(
      client_connection->connection, MHD_CONNECTION_INFO_GNUTLS_SESSION);
      ...
      if(tls_session == nullptr) {
        ...
        client_connection->server->SendResponse(
            "No active SSL session", client_connection);
      }

      unsigned int client_cert_status;
      gnutls_certificate_verify_peers2(tls_session, &client_cert_status);
      if (client_cert_status != 0) {
        client_connection->code = 401;
        client_connection->server->SendResponse("Unauthorized invalid peer SSL certificate", client_connection);
      }
